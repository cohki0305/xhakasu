...
5. 台帳にない分を並列で `env.AI.run('typesafe/jev', …)` に渡す。結果を `cache:<key>` に TTL 30 日で書く。台帳と件数の書き込みは `ctx.waitUntil` で返事の後に回す。
...
## 試し打ちの結果（2026-09-19 実施）

正解ラベルつきの日本語の投稿 30 件を、初期 7 ジャンル＋攻撃的＋性的の 3 問で `jev-1.13.0` に投げた（REST API、5 並列、アカウントは cohki0305）。

| 項目 | 結果 |
|---|---|
| ジャンル | 29/30 正解。外した 1 件は「AI エージェントで営業を置き換えた話」で tech 0.52 / business 0.48。人が見ても割れる投稿 |
| 攻撃的 | 該当 4 件は 0.92〜0.98、非該当 26 件は 0.01〜0.09 |
| 性的 | 該当 3 件は 0.91〜0.97、非該当 27 件は 0.01〜0.05 |
| 応答時間 | 中央 335 ms、最大 1,018 ms |
| トークン | 1 投稿あたり入力 588、出力 107（平均） |

日本語で実用になる。該当と非該当の確率が大きく離れているので、閾値の初期値（0.85 / 0.70 / 0.50）はどれでも同じ結果になる。

本番の中継 Worker 経由の実測（2026-09-19、台帳にない 10 件の束 × 3 回）: 0.9〜1.1 秒。KV への書き込み（台帳・件数）を返事の後に回す（`ctx.waitUntil`）前は 2.1〜2.6 秒だった。台帳に全件ある束は 0.1 秒未満。拡張のタイムアウト 4 秒に対して余裕がある。

残る不確かさ:

- 試した 30 件は書き言葉の整った文面で、実際の投稿にある皮肉、スラング、極端に短い文、伏せ字は含まない。実装後に実タイムラインで見直す。
- 割れる投稿は確率も割れる。`decide` が「1 位のジャンル」ではなく「見たいジャンルの確率の合計」を見る設計は、この結果と合っている。
- 料金は未確認（ダッシュボードで確認する）。1 投稿あたり約 700 トークン。
